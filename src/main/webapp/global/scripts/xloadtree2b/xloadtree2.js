...
_p.getIconSrc = function () {
	...
	if (this.hasChildren()) {
		return open ? webFXTreeConfig.folderIcon : webFXTreeConfig.folderIcon;
	}
	return webFXTreeConfig.fileIcon;
...
WebFXLoadTree._attrs = ["text", "src", "action", "id", "target", "isLeaf"];
...
WebFXLoadTree.createItemFromElement = function (oNode) {
	...
	for (i = 0; i < l; i++) {
		if (!isLeaf && cs[i].tagName == "tree") {
			...
			WebFXLoadTree.loadNodeInfo(jsNode, cs[i].childNodes);
		}
...
WebFXLoadTree.loadNodeInfo = function(jsNode, infos){
	var l = infos.length;
	for(var i=0; i<l; i++) {
		var info = infos[i];
