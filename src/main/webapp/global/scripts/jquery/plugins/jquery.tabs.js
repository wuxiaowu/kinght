        ...
        var tabs = $('a', nav);
		 ...
		 if (settings.iframe) {
            tabs.each(function() {
				...
				if(this.id)iframeId = 'iframe-' + this.id;
				...
				var getIframeId = function(el){
					if(el.id) return 'iframe-' + el.id;
					return '';
				};

				var genIframeId = function(el){
					var iframeId = getIframeId(el);
					
					if(iframeId != '') return ' id = "' + iframeId + '"';
					else return '';
				};
				
				var getIframeSrc = function(el){
					var url = $(el).attr('url');
					return url ? url : '';
				};
				
				var genIframeSrc = function(el){
					var url = getIframeSrc(el);
                ...
                $(this).bind('loadRemoteTab', function(e, callback) {
                    ...
                    setTimeout(function() { // Timeout is again required in IE, "wait" for id being restored
						...
						if($iframe.length == 0)$(hash).html('<iframe' + genIframeId(myselfEl) + ' marginwidth="0" marginheight="0" frameBorder="0" id="edit-iframe"' + genIframeSrc(myselfEl) +'></iframe>');
						else{
							$iframe.attr('src', getIframeSrc(myselfEl));
							$(hash).html($iframe.get(0));
						}
						
//						$(hash)
//							.html('<iframe' + genFrameId(myselfEl) + ' marginwidth="0" marginheight="0" frameBorder="0"' + genFrameSrc(myselfEl) +'></iframe>')
//							.siblings('div.tabs-container').empty();
						 if (settings.spinner) {
                ...
                $(this).bind('loadRemoteTab', function(e, callback) {
                    ...
                    setTimeout(function() { // Timeout is again required in IE, "wait" for id being restored
                        $(hash).load(url, function() {
                            ...
                            $$.removeClass(settings.loadingClass);
                            callback && callback();
							settings.onLoad && settings.onLoad($(hash));
                        });
                    }, 0);
                });
        ...
        if (settings.disabled && settings.disabled.length) {
            for (var i = 0, k = settings.disabled.length; i < k; i++) {
				var v = --settings.disabled[i];
                tabs.slice(v, v+1).trigger('disableTab').end();
            }
        };
        ...
        tabs.bind('click', function(e) {
            ...
            if (toShow.size()) {
                ...
                if (settings.remote || settings.iframe) {
        ...
        return function(tab) {
            return this.each(function() {
                ...
                if (!tab || typeof tab == 'number') {
					var v = tab && tab > 0 && tab - 1 || 0;
                    a = $('li a', nav).slice(v, v+1); // fall back to 0
...
$.fn.activeIframe = function() {
    return $('div.tabs-container > iframe', this);
};

$.fn.activeIframeUrl = function(url) {
	$('ul.tabs-nav > li.tabs-selected > a', this).attr('url', url);
	this.activeIframe().attr('src', url);
};
