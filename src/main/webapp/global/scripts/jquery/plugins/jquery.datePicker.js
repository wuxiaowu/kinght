	...
	$.extend(
		...
		{	
			...
			display : function(eleAlignTo)
			{
				...
				if($.ss.isIe6()) $('#dp-popup').prepend('<iframe class="repair-select" frameborder="0" src="about:blank"></iframe> ')
				var obj,x,y,dx,dy;
				if ($.browser.mozilla) {
					$("#dp-popup").mousedown( function(event) {
					  obj = (event.target) ? event.target: event.srcElement;
					});
				}
				if ($.browser.msie) {
					  $("#dp-popup h2").mousedown( function(event) {
					  obj = (event.target) ? event.target: event.srcElement;
					});
				}

				$('#dp-popup').mousemove( function(event) { 
					if ($.browser.mozilla) {
						x = event.pageX;
						y = event.pageY;
					}
					...
					if(obj)$('#dp-popup').css({ left: x- 100, top: y- 16,cursor: 'move' });
				});
				
				$("#dp-popup").mouseup( function(event) {
					  if (!obj) return;
					  obj = false; 
