...
function isMobile( arrEle ) {
	return execRegExpForEles( arrEle,/^((\(\d{2,3}\))|(\d{3}\-))?\d{11}$/,"格式不正确。\n举例：13975986315" );
}
//判断一组表单域的值是否为邮政编码。
...
function isLimit(limit,mode) {
	...
	for( var i = 0;i < eles.length;i ++ ) {
		...
		if( eleValue < minValue ){
			alert(eleDesc + "必须大于" + minValue );
			if( eleInfo.object.focus() ) eleInfo.object.focus();
			return false;
		}
		if( eleValue > maxValue ) {
			alert(eleDesc + "必须小于" + maxValue );
			if( eleInfo.object.focus() ) eleInfo.object.focus();
			return false;
		}
	}
...
function getObjInfo(ele) {
	...
	//得到描述文本
	var objDesc = document.getElementsByName(ele + "_tip")[0];
